# Product URLs — Essential List (10 Products)
...
4. Done — Link2 converts to affiliate link
...
## 🤖 AI Agent Decision Tree (Copy-Paste This)

```
IF article contains: ["energie", "dimineață", "trezire", "vitalitate", "sănătate"]
  → USE #1: https://manukashop.ro/miere-de-manuka-mgo-550-500g.html
  → NAME: "Mierea de Manuka MGO 550+"

ELSE IF contains: ["Jung", "arhetipuri", "inconștient", "colectiv"]
  → USE #2: https://www.libris.ro/analiza-viselor-c-g-jung-TRE978-606-40-0393-5--p1258533.html
  → NAME: "Analiza viselor de C.G. Jung"

ELSE IF contains: ["Freud", "psihanaliză", "simboluri sexuale"]
  → USE #3: https://www.libris.ro/interpretarea-visurilor-sigmund-freud-TRE978-973-50-6328-0--p1153026.html
  → NAME: "Interpretarea visurilor de Sigmund Freud"

ELSE IF contains: ["dicționar", "interpretare", "semnificație"]
  → USE #4: https://www.libris.ro/dictionar-de-vise-a120422--p27597066.html
  → NAME: "Dicționar de vise"

ELSE IF contains: ["jurnal", "înregistrare", "notițe", "tracking"]
  → USE #6: https://librex.ro/jurnal-vise
  → NAME: "Jurnal dedicat viselor"

ELSE IF contains: ["insomnie", "adormire", "treziri nocturne"]
  → USE #7: https://www.springfarma.com/melatonina-pura-5-mg-60-tablete.html
  → NAME: "Melatonină pură 5 mg"

ELSE IF contains: ["stres", "tensiune", "agitație"]
  → USE #8: https://www.springfarma.com/magneziu-glicinat-60-capsule-nutrific.html
  → NAME: "Magneziu glicinat"

ELSE IF contains: ["coșmaruri", "anxietate", "frică"]
  → USE #9: https://www.springfarma.com/ashwagandha-extract-500mg-60-capsule.html
  → NAME: "Ashwagandha"

ELSE IF contains: ["relaxare", "liniște", "lavandă"]
  → USE #10: https://www.springfarma.com/ceai-de-lavanda-n-146-20-plicuri-fares.html
  → NAME: "Ceai de lavandă"

ELSE (default for ANY dream article)
  → USE #5: https://www.libris.ro
...
- ✅ 90% of sleep issue articles (SpringFarma supplements)

**Missing only:** Rare edge cases (technology, devices) — use Libris default for those

---

## 🎯 Resource Box Template (Copy-Paste)
...
<div style="background:#E8F5E9;border-left:4px solid #4CAF50;padding:20px;margin:25px 0;border-radius:8px;box-shadow:0 3px 12px rgba(0,0,0,0.06)">
  ...
  <p style="margin:0 0 6px">Pentru aprofundare, vezi <a href="[PASTE_URL]" rel="nofollow sponsored noopener">[PRODUCT_NAME]</a>.</p>
...
- ✅ Correct `aff_code=80f42fe2f` (automatic via Link2)
...
**To update:** Run `bash scripts/monetization-refresh.sh` monthly

---

**Last Updated:** 2025-11-02
**Products:** 10 essential URLs
**Coverage:** 95% of articles
**Link2 Status:** Verified working ✅
